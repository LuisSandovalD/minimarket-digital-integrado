// ============================================================================
// prisma/seeds/sale.seed.2023.js
// Ventas distribuidas exclusivamente en el año 2023 - Multiempresa y Multisucursal
// Consolidación comercial implementada mediante volumen masivo de transacciones
// ============================================================================
...
// Se vuelven a incrementar los ratios para simular la consolidación del negocio en 2023
// (2021: 1.2-2.0 | 2022: 1.8-2.8 | 2023: 2.4-3.6)
const SALE_TO_PRODUCT_RATIO_MIN = 2.4;
const SALE_TO_PRODUCT_RATIO_MAX = 3.6;
...
const poolNotes = [
    ...
    "Venta por canal digital con recojo en tienda",
    "Atención a cliente corporativo - Campaña anual",
    "Venta mayorista con descuento por volumen comercial",
    "Pedido integrado a través de la plataforma B2B"
];
...
async function saleJsonPayloadSeed2023() {
    console.log(`🚀 Iniciando carga masiva de Ventas exclusivamente del año ${YEAR} (Año de Consolidación) para todas las empresas...`);
    ...
    for (const currentCompany of allCompanies) {
        ...
        // CORRECCIÓN MULTISUCURSAL: Traemos todas las sucursales asignadas a la empresa
        ...
        // Validación preventiva estricta
        ...
        // Volumen de ventas escalado según los nuevos ratios de alta demanda para 2023
        const ratio = SALE_TO_PRODUCT_RATIO_MIN + Math.random() * (SALE_TO_PRODUCT_RATIO_MAX - SALE_TO_PRODUCT_RATIO_MIN);
        const totalRecords = Math.max(120, Math.floor(allProducts.length * ratio)); // Mínimo incrementado a 120

        console.log(`🏢 Empresa: "${currentCompany.name}" → Generando ${totalRecords} ventas [${YEAR}] distribuidas en sus sucursales...`);
        ...
        for (let i = 0; i < totalRecords; i++) {
            ...
            // Selección dinámica de la sucursal donde ocurre la transacción
            const targetBranch = allBranches[Math.floor(Math.random() * allBranches.length)];

            // En 2023 los tickets son más robustos (hasta 6 productos distintos por venta)
            const detailCount = Math.floor(Math.random() * 6) + 1;
            const selectedProducts = shuffleArray(allProducts).slice(0, detailCount);

            const rawDetails = selectedProducts.map(p => {
                const quantity = Math.floor(Math.random() * 15) + 1; // Hasta 15 unidades por línea
            ...
            // Validar stock ANTES de abrir la transacción apuntando a la sucursal destino
            ...
            await prisma.$transaction(async (tx) => {
                ...
                const esCredito = Math.random() < 0.20; // En 2023 se optimiza la liquidez (80% al contado)
                const status = esCredito ? "PENDING" : "COMPLETED";
                const porcentaje = esCredito ? (0.30 + Math.random() * 0.30) : 1.0; // Mayor inicial para créditos
    ...
    console.log(`\n======================================================`);
    console.log(`✅ [${YEAR}] SEED COMPLETADO: ventas inyectadas con máxima densidad comercial.`);
...
if (require.main === module) {
    saleJsonPayloadSeed2023()
        .catch((e) => {
            console.error("❌ Error detectado en el proceso de seed de ventas [2023]:", e);
            process.exit(1);
        })
        .finally(async () => {
...
module.exports = { saleJsonPayloadSeed2023 };