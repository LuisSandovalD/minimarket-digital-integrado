...
exports.generateTopProductsExcel = async ({ report, company, imgBuf }) => {
  ...
  if (imgBuf?.buffer) {
    try {
      ...
      /* Se ignora de forma intencional si el buffer de la imagen falla */
    }
  }
  ...
  (report || []).forEach((r, index) => {
    ...
    // Solo incrementamos si quedan más elementos por procesar para evitar "no-useless-assignment"
  ...
  const dataEndRow2 = currentExcelRow2;

  // Apuntamos a la siguiente fila para escribir la fila de totales finales
  currentExcelRow2++;
