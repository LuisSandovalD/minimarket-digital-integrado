  ...
  const renderRows = (data = [], isChild = false) => (
    <>
      {data.map((category) => (
        <tr
          key={category.id}
          className="border-b border-slate-200/60 transition-all duration-200 hover:bg-slate-50/70 dark:border-slate-800 dark:hover:bg-slate-900/40"
        >
          {/* CATEGORÍA */}
          <td className="px-6 py-5">
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900">
                {isChild ? (
          ...
          {/* DESCRIPCIÓN */}
          <td className="px-6 py-5">
            <p className="max-w-[280px] text-sm leading-relaxed text-slate-600 dark:text-slate-300">
    ...
    <div className="space-y-8">
      ...
      {parentCategories.length > 0 && (
        <div>
          ...
          <Table>
            ...
            <tbody>{renderRows(parentCategories)}</tbody>
          </Table>
        </div>
      )}
      ...
      {childCategories.length > 0 && (
        <div>
          ...
          <Table>
            <THead columns={columns} />
            <tbody>{renderRows(childCategories, true)}</tbody>
          </Table>
        </div>
      )}
