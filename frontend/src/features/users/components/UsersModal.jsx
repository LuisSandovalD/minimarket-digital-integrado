...
export default function UserModal({
  ...
  const { availableManagers, handleSelectManager, isBranchInherited } =
    useUserModalData({ formData, allUsers, isEdit, user, handleChange });
  ...
  return (
    <Modal open={open} onClose={onClose} size="full">
      ...
      >
        {/* CONTENEDOR PRINCIPAL CON RANGO DE RESPIRACIÓN (`space-y-5`) */}
        <div className="max-h-[72vh] overflow-y-auto px-6 py-5 space-y-5">
          {/* SECCIÓN SUPERIOR: AVATAR + DATOS GENERALES */}
          <div className="grid gap-5 lg:grid-cols-[350px_1fr]">
            {/* AVATAR CON AIRE */}
            <div className="flex flex-col items-center justify-center p-5 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
              <ModernImageUpload
            ...
            {/* FORMULARIO CÓMODO (Grid balanceado de 2 columnas) */}
          ...
          {/* SECCIÓN CENTRAL: SELECTOR DE ROL */}
          ...
          {/* SECCIÓN RESPONSABLE (Tarjetas con mejor tracking y padding) */}
          {formData.role !== "ADMIN" && (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/40 p-5">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-3">
              ...
              {availableManagers.length > 0 ? (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {availableManagers.map((mgr) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-100 dark:bg-slate-950 text-slate-500-hidden border border-slate-200/60 dark:border-slate-800">
                          {mgr.avatar ? (
                            <img
                              src={mgr.avatar}
                              alt={mgr.name}
                ...
                <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-800 p-4 bg-white/50 dark:bg-slate-900/20">
                  <p className="text-xs text-slate-400 dark:text-slate-500 italic">
                    No hay superiores jerárquicos disponibles para este rol en
                    el sistema.
                  </p>
                </div>
              )}
                ...
                branches.map((branch) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        handleChange({
                          target: { name: "branchId", value: branchId },
                        })
                      }
                      className={`relative rounded-xl flex items-center gap-2.5 border px-3 py-3 text-left transition-all duration-150
                        ...
                        ${isBranchInherited ? "opacity-40 cursor-not-allowed" : "hover:bg-slate-50 dark:hover:bg-slate-850"}`}
                    >
                      <Building2
                        size={14}
                        className={
        ...
        {/* PIE DE PÁGINA (Footer con aire inferior limpio) */}
        <FooterModal>
