...
export default function SalesReportPage({ filters }) {
  ...
  const fetchPdf = async () => {
    ...
    try {
      ...
      setError(err.message || "Error al generar el reporte de ventas.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Visualiza de manera oficial el balance y las operaciones de salida correspondientes al rango de fechas
            seleccionado.
          </p>
        </div>
