...
export default function InventoryReportPage({ filters }) {
  ...
  useEffect(() => {
    const fetchPdf = async () => {
      try {
        ...
        if (filters && filters.startDate && filters.endDate) {
          ...
          // 1. Solicitamos el binario (Blob) del PDF de inventario
          ...
          // 2. Generamos la URL local para el objeto Blob
        ...
        setError(
          "Ocurrió un error al generar la vista previa del reporte de inventario.",
        );
      } finally {
        setLoading(false);
      }
    ...
    // Limpieza de memoria (Clean-up) cuando el usuario sale de la página
    return () => {
  ...
  // Mostrar esqueleto durante la carga asíncrona
  if (loading) return <SkeletonTable />;

  // Renderizado en caso de error en la petición HTTP o conversión del blob
  if (error) {
    return (
  ...
  return (
    <div className="space-y-4">
      {/* Encabezado del Reporte de Inventario */}
      <div>
      ...
      <div className="w-full h-[750px] bg-slate-100 dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm">
        {pdfUrl ? (
          <iframe
            src={`${pdfUrl}#toolbar=1`} // Habilita herramientas integradas del navegador (Descarga, impresión, zoom)
