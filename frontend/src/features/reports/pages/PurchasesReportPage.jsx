...
export default function PurchasesReportPage({ filters }) {
    ...
    const fetchPdf = async () => {
      try {
        ...
        if (filters && filters.startDate && filters.endDate) {
          ...
          // 1. Solicitamos el binario (Blob) del PDF de compras
          ...
          // 2. Generamos la URL local segura para el iframe
        ...
        setError(
          "Ocurrió un error al generar la vista previa del reporte de compras.",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="space-y-4">
      {/* Encabezado del Reporte de Compras */}
      <div>
      ...
      <div className="w-full h-[750px] bg-slate-100 dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm">
        {pdfUrl ? (
          <iframe
            src={`${pdfUrl}#toolbar=1`} // Activa las herramientas de descarga e impresión integradas por el navegador
