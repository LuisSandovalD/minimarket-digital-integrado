...
export default function useReports() {
  ...
  const handleDownload = async ({ format }) => {
    ...
    const params = {
      ...
      startDate: useFilters.startDate
        ? formatDateISO(useFilters.startDate)
        : undefined,
      endDate: useFilters.endDate
        ? formatDateISO(useFilters.endDate)
        : undefined,
    };
    ...
    function resolveEndpoint(type, fmt) {
      if (type === "sales") {
        return fmt === "excel"
          ? `/sale/reports/daily/excel`
          : `/sale/reports/daily/pdf`;
      }
      if (type === "top-products") {
        return fmt === "excel"
          ? `/sale/reports/top-products/excel`
          : `/sale/reports/top-products/pdf`;
      }
      if (type === "purchases") {
        return fmt === "excel"
          ? `/purchase/reports/daily/excel`
          : `/purchase/reports/daily/pdf`;
      }
      if (type === "inventory") {
        return fmt === "excel"
          ? `/inventory/reports/inventory/excel`
          : `/inventory/reports/inventory/pdf`;
      }
      if (type === "customers") {
        return fmt === "excel"
          ? `/customer/reports/customers/excel`
          : `/customer/reports/customers/pdf`;
      }
      if (type === "suppliers") {
        return fmt === "excel"
          ? `/supplier/reports/suppliers/excel`
          : `/supplier/reports/suppliers/pdf`;
      }
      if (type === "payments") {
        return fmt === "excel"
          ? `/payments/reports/payments/excel`
          : `/payments/reports/payments/pdf`;
      }
      if (type === "audit" || type === "activity") {
        return fmt === "excel"
          ? `/dashboard/reports/activity/excel`
          : `/dashboard/reports/activity/pdf`;
      }
    ...
    try {
      ...
      if (format === "excel") {
        ...
        if (!endpoint) {
          alert(
            "Exportación a Excel no disponible para el tipo de reporte seleccionado.",
          );
          return;
        }
        ...
        const startLabel = useFilters.startDate
          ? formatDateISO(useFilters.startDate)
          : "sin-fecha";
        const endLabel = useFilters.endDate
          ? formatDateISO(useFilters.endDate)
          : null;
      ...
      if (format === "pdf") {
        ...
        if (!endpoint) {
          alert(
            "Exportación a PDF no disponible para el tipo de reporte seleccionado.",
          );
          return;
        }
