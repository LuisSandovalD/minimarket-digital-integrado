...
export default function ConfigurationPage() {
  ...
  const { form, loading, saving, errors, updateField, saveConfiguration } = useConfiguration();
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <AppearanceSection form={form} updateField={updateField} errors={errors} />
        <FinanceSection form={form} updateField={updateField} errors={errors} />
        <InventorySection form={form} updateField={updateField} errors={errors} />
        <SecuritySection form={form} updateField={updateField} errors={errors} />
        ...
        <div className="pt-4 border-t border-slate-200/60 dark:border-white/10">
          <button
            ...
            className={`w-full md:w-auto md:px-12 rounded-xl bg-slate-950 dark:bg-slate-100 p-3.5 text-sm font-medium text-white dark:text-slate-950 transition-all duration-200 shadow-md ${
              saving ? "opacity-50 cursor-not-allowed scale-[0.99]" : "hover:opacity-90 active:scale-[0.98]"
            }`}
          >
            {saving ? "Guardando cambios en el sistema..." : "Guardar todas las configuraciones"}
          </button>
        </div>
      </form>
    </div>
