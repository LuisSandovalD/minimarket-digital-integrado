...
export const usePaymentMetrics = (payments = []) => {
  ...
  const totalAmount = safePayments.reduce((total, payment) => {
    const amount = payment.amount ?? payment.totalAmount ?? payment.total ?? payment.paymentAmount ?? 0;
  ...
  const completed = safePayments.filter((payment) => {
    ...
    return ["COMPLETED", "PAID", "SUCCESS", "SUCCESSFUL", "PAGADO", "COMPLETADO"].includes(status);
  }).length;
