...
export default function FlowGrid({ flows = [], columns = 3, animate = true }) {
  ...
  const gridItems = flows.flatMap((flow) => {
    ...
    const featuresItems = Object.entries(flow.features || {}).map(
      ([category, items], index) => ({
        ...
        title: category
          .replace(/_/g, " ")
          .replace(/\b\w/g, (c) => c.toUpperCase()), // Nombre limpio de la subcategoría
        description: (
              ...
              description: (
                <div className="grid grid-cols-1 gap-2.5 mt-3">
                  {flow.stats.map((stat) => (
                    ...
                    >
                      <div className="text-xs text-muted-foreground pr-2 font-medium leading-tight">
                        {stat.label}
                      </div>
