...
import { SUBSCRIPTION_FLOW } from "../constants/automationsData.js";
import FlowGrid from "./FlowGrid.jsx";

// Variantes de animación idénticas para mantener la consistencia
const fastStagger = {
  hidden: { opacity: 0 },
  show: {
    opacity: 1,
...
export default function SubscriptionFlow() {
  const Icon = SUBSCRIPTION_FLOW.icon;
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        >
          <Icon size={14} />
          Módulo de Ingresos Recurrentes
        </motion.div>
        ...
        >
          {SUBSCRIPTION_FLOW.description}
        </motion.p>
      </motion.div>

      {/* Grid de Tarjetas por Columnas (Configurado a 4 columnas) */}
      <FlowGrid flows={[SUBSCRIPTION_FLOW]} columns={4} animate={true} />
    </div>
  );
}
