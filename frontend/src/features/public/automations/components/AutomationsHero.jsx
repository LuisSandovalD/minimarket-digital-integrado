...
// Importamos el flujo y además el ícono dinámico que le asignamos al motor
import { AUTOMATIONS_FLOW } from "../constants/automationsData";

export default function AutomationsHero() {
  // Obtenemos el icono del flujo (el componente Cpu que asignamos antes)
  const FeatureIcon = AUTOMATIONS_FLOW.icon;
        ...
        >
          ...
          <motion.div variants={fastFadeUp} className="mt-8 flex flex-wrap gap-3">
            {Object.keys(AUTOMATIONS_FLOW.features || {}).map((categoryKey) => (
              ...
              >
                <FeatureIcon className="h-4 w-4 text-[#274c77] dark:text-[#a3cef1]" />
                <span className="text-sm font-medium text-[#0f172a] dark:text-white">
                  {formatTagLabel(categoryKey)}
                </span>
              </div>
            ))}
          </motion.div>
