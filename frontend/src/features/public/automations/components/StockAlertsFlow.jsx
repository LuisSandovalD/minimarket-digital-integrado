...
import { STOCK_FLOW } from "../constants/automationsData.js";
import FlowGrid from "./FlowGrid.jsx";

// Variantes de animación idénticas para una experiencia visual homogénea
const fastStagger = {
  hidden: { opacity: 0 },
  show: {
    opacity: 1,
...
export default function StockAlertsFlow() {
  const Icon = STOCK_FLOW.icon;
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        >
          <Icon size={14} />
          Módulo de Logística y Existencias
        </motion.div>
        ...
        >
          {STOCK_FLOW.description}
        </motion.p>
      </motion.div>

      {/* Grid de Tarjetas por Columnas (Configurado a 4 columnas) */}
      <FlowGrid flows={[STOCK_FLOW]} columns={4} animate={true} />
    </div>
  );
}
