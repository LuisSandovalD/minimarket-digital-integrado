import { Calendar, DollarSign, Layers, Minus, Package, Plus, ShoppingCart, Trash2 } from "lucide-react";
...
export default function PurchaseCartModal({
  ...
  return (
    <Modal open={open} onClose={onClose} size="xl">
      <HeaderModal title="Productos de la Compra" subtitle={`${totalUnits} productos agregados`} onClose={onClose} />
      ...
      <div className="min-h-[500px] max-h-[65vh] overflow-y-auto p-6 bg-slate-50/30 dark:bg-slate-900/10">
        {details.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center py-20">
            ...
            <h3 className="mt-4 font-semibold text-slate-700 dark:text-slate-300">Sin productos</h3>
            <p className="text-sm text-slate-400 mt-2">Agrega productos para continuar.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4 border-b border-slate-100 dark:border-slate-900 pb-3">
                  <div className="flex-1">
                    <h4 className="font-semibold text-sm text-slate-900 dark:text-slate-100">{item.productName}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                  {/* CANTIDAD */}
                  <div>
                    <label className="text-xs font-medium text-slate-400 dark:text-slate-500">Cantidad</label>
                  ...
                  {/* COSTO */}
                  <div>
                    <label className="text-xs font-medium text-slate-400 dark:text-slate-500">Costo Compra</label>
                    <div className="relative mt-1">
                      ...
                      <input
                        ...
                        onChange={(e) => updateCostPrice(item.productId, e.target.value)}
                  ...
                  {/* SUBTOTAL */}
                  <div>
                    <label className="text-xs font-medium text-slate-400 dark:text-slate-500">Subtotal</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-dashed border-slate-100 dark:border-slate-900">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateBatchNumber(item.productId, e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateExpirationDate(item.productId, e.target.value)}
                ...
                <div className="mt-3.5 flex items-center gap-2 text-xs text-slate-400 dark:text-slate-500">
                  ...
                  <strong className="text-slate-600 dark:text-slate-400">{item.stock || 0} und.</strong>
                </div>
              </div>
            ))}
          </div>
      ...
      <FooterModal>
        <div className="flex items-center justify-between w-full">
          <div>
            ...
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{fmt(totalAmount)}</p>
          </div>
