...
export default function CompanyStep({ form, handleChange }) {
  ...
  useEffect(() => {
    if (!form.subscriptionTier) {
      handleChange({ target: { name: "subscriptionTier", value: "FREE" } });
    }
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 focus:outline-none"
          >
          ...
          {plans.map((plan) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  handleChange({
                    target: { name: "subscriptionTier", value: plan.value },
                  })
                }
                className={`relative rounded-2xl border p-5 text-left flex flex-col justify-between min-h-[145px] w-full ${isSelected ? "border-slate-900 bg-slate-900 text-white shadow-md dark:border-slate-100 dark:bg-slate-100 dark:text-slate-950" : "border-slate-200 bg-white text-slate-700 hover:border-slate-400 dark:border-white/10 dark:bg-white/[0.02] dark:text-slate-300"}`}
              >
                <div className="w-full flex flex-col justify-between h-full space-y-2">
                  ...
                  <p
                    className={`text-xs leading-relaxed ${isSelected ? "text-slate-300 dark:text-slate-600" : "text-slate-500 dark:text-slate-400"}`}
                  >
                    {plan.description}
                  </p>
                </div>
        ...
        <div
          className={`mt-3 p-3.5 rounded-xl border flex items-start gap-3 ${currentTier !== "FREE" ? "border-indigo-100 bg-indigo-50 text-indigo-900 dark:border-indigo-950/30 dark:bg-indigo-950/10 dark:text-indigo-300" : "border-slate-100 bg-slate-50 text-slate-600 dark:border-slate-800/50 dark:bg-slate-900/20 dark:text-slate-400"}`}
        >
          {currentTier !== "FREE" ? (
            <>
