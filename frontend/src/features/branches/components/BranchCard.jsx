...
// ========================================
// Minimal Premium Card - Inspired by: Linear, Raycast, Vercel
// ========================================
...
export default function BranchCard({ branch, onEdit, onDeleteTrigger }) {
  // 🌟 Recuperamos el usuario de la sesión actual y validamos si es administrador
  const user = getUser();
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-slate-200/70 bg-white transition-all duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-xl hover:shadow-black/[0.04] dark:border-slate-800 dark:bg-slate-950 dark:hover:border-slate-700">
      ...
      <div className="relative z-10 flex h-full flex-col p-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0">
              <h3 className="truncate text-lg font-medium tracking-tight text-slate-900 dark:text-slate-100">
                {branch.name}
              </h3>
              {branch.code && (
                <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium uppercase tracking-wide text-slate-500 dark:bg-slate-900 dark:text-slate-400">
                  <Code size={12} />
                  {branch.code}
                </div>
              )}
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 🌟 El botón de eliminar solo se renderiza si el rol guardado en la sesión es estrictamente 'admin' */}
            {isAdmin && (
              <ModernButton
                text=""
                icon={Trash2}
                variant="danger"
                size="sm"
                onClick={() => onDeleteTrigger?.(branch)}
                className={`${actionButtonStyles} text-rose-500 hover:border-rose-500/20 hover:bg-rose-500/10 hover:text-rose-400`}
              />
            )}
          </div>
        </div>
        ...
        {/* FOOTER */}
        <div className="mt-8 flex items-center justify-between border-t border-slate-200/70 pt-5 dark:border-slate-800">
