import { BadgeCheck, Building2, Globe, Mail, MapPin, Phone, ReceiptText, ShieldCheck, User2 } from "lucide-react";
  ...
  return (
    <section className="mt-6 rounded-2xl border border-slate-200/60 bg-white/60 p-6 backdrop-blur-md transition-all duration-300 hover:border-slate-300/80 dark:border-slate-800/60 dark:bg-slate-950/60 dark:hover:border-slate-700/80 md:p-7">
      ...
      <div className="mb-6 border-b border-slate-100 pb-4 dark:border-slate-900">
        <h3 className="text-sm font-semibold tracking-tight text-slate-900 dark:text-slate-100">Información General</h3>
      ...
      <div className="grid gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => {
          ...
          return (
            <div key={item.label} className="group flex items-start gap-3.5 min-w-0 py-1">
              {/* ICON */}
              ...
              <div className="min-w-0 flex-1 space-y-0.5">
                ...
                <div className="break-words text-xs font-medium leading-relaxed text-slate-700 dark:text-slate-300">
                  {item.value || (
                    <span className="text-slate-400/70 dark:text-slate-600 font-normal italic">No registrado</span>
                  )}
                </div>
              </div>
            </div>
